...
class TestGATEncoder:
    ...
    @pytest.mark.torch
    def test_gat_encoder_initialization_default(self):
        """Test GATEncoder initialization with default parameters."""
        with patch('molenc.encoders.representations.graph.gat.GATEncoder._init_model'):
            from molenc.encoders.representations.graph.gat import GATEncoder
    ...
    @pytest.mark.torch
    def test_gat_encoder_initialization_custom(self):
        """Test GATEncoder initialization with custom parameters."""
        with patch('molenc.encoders.representations.graph.gat.GATEncoder._init_model'):
            from molenc.encoders.representations.graph.gat import GATEncoder
            ...
            assert encoder.hidden_dims == [128, 256]
            assert encoder.output_dim == 256
    ...
    @pytest.mark.torch
    @pytest.mark.rdkit
    def test_gat_encode_single_valid_smiles(self, sample_smiles):
        """Test encoding a single valid SMILES string."""
        from molenc.encoders.representations.graph.gat import GATEncoder
        
        # Mock the model and graph conversion
        with patch.object(GATEncoder, '_init_model') as mock_init_model:
            with patch.object(GATEncoder, '_mol_to_graph') as mock_graph:
                # Mock graph data
                mock_graph.return_value = (
                    torch.randn(10, 9),  # node_features: 10 atoms, 9 features each
                    torch.randint(0, 2, (10, 10)).float()  # adjacency_matrix: 10x10
                ...
                # Mock the model's forward method
                mock_model.return_value = torch.randn(10, 64)  # 10 nodes, 64 features each
                
                embedding = encoder.encode(sample_smiles[0])
                
                assert isinstance(embedding, np.ndarray)
                assert embedding.shape == (64,)  # Should be pooled to single vector
                assert embedding.dtype == np.float32 or embedding.dtype == np.float64
    
    def test_gat_encoder_missing_dependencies(self):
        """Test GATEncoder raises error when dependencies are not available."""
        with patch.dict('sys.modules', {'torch': None}):
            with pytest.raises(DependencyError) as exc_info:
                from molenc.encoders.representations.graph.gat import GATEncoder
                GATEncoder()
            assert "torch" in str(exc_info.value)
            assert "gat" in str(exc_info.value)
