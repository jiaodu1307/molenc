...
@require_dependencies(['requests'], 'HttpChemBERTa')
class HttpChemBERTaEncoder(BaseEncoder):
    def __init__(self,
        ...
        if not self.base_url:
            raise EncoderInitializationError('HttpChemBERTa', 'base_url is required for HTTP remote encoder')
    ...
    def _encode_single(self, smiles: str) -> np.ndarray:
        vec = self._client.encode_single(smiles, encoder_type='chemberta', model_name=self.model_name)
    ...
    def encode_batch(self, smiles_list: List[str]) -> np.ndarray:
        encodings = self._client.encode_batch(smiles_list, encoder_type='chemberta', model_name=self.model_name)
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        cfg.update({'base_url': self.base_url, 'timeout': self.timeout, 'remote': True, 'model_name': self.model_name})
        return cfg