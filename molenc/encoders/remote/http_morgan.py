"""HTTP remote Morgan encoder implementation."""
...
@require_dependencies(['requests'], 'HttpMorgan')
class HttpMorganEncoder(BaseEncoder):
    """Morgan fingerprint encoder via HTTP remote service."""
    ...
    def __init__(self,
        ...
        if not self.base_url:
            raise EncoderInitializationError('HttpMorgan', 'base_url is required for HTTP remote encoder')

        self._client = CloudAPIClient(APIConfig(
    ...
    def _encode_single(self, smiles: str) -> np.ndarray:
        vec = self._client.encode_single(smiles, encoder_type='morgan')
    ...
    def encode_batch(self, smiles_list: List[str]) -> np.ndarray:
        encodings = self._client.encode_batch(smiles_list, encoder_type='morgan')
    ...
    def get_output_dim(self) -> int:
        ...
        # Fallback: try a tiny probe if possible (not invoked automatically)
        return 2048
